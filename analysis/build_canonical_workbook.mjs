...
readme.getRange("A10:B17").values = summaryRows;
readme.getRange("A9:B9").format = { fill: green, font: { bold: true, color: "#FFFFFF" } };
readme.getRange("A9:B17").format.borders = { preset: "all", style: "thin", color: border };
readme.getRange("B16:B17").format.numberFormat = "0.0%";

readme.getRange("A19:D19").merge();
readme.getRange("A19").values = [["How repeated rows were handled"]];
readme.getRange("A19:D19").format = { fill: lime, font: { bold: true, color: green } };
readme.getRange("A20:D24").merge();
readme.getRange("A20").values = [[
  "The 119 repeated building-days contain identical production values but multiple environmental readings. Canary keeps the first available production value, takes the minimum of minimum readings, maximum of maximum readings, and mean of average readings. The Duplicate Audit sheet preserves all 238 source rows so the consolidation can be checked directly.",
]];
readme.getRange("A20:D24").format = { wrapText: true, verticalAlignment: "top", fill: pale };
readme.getRange("A26:D26").values = [["Sheet", "What it contains", "Rows", "Use"]];
readme.getRange("A27:D29").values = [
...
readme.getRange("A26:D26").format = { fill: green, font: { bold: true, color: "#FFFFFF" } };
readme.getRange("A26:D29").format.borders = { preset: "all", style: "thin", color: border };
readme.getRange("A1:D29").format.autofitColumns();

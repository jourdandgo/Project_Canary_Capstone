...
> Create a slide titled “Recovery: predict remaining loss, then apply strict deployment gates.” State training Y: additional population loss after the review date; final forecast = current survival − predicted additional loss. Use this table: age-band baseline — MAE 1.40 pts, cycle MAE 1.50, RMSE 1.80, R² 0.222; linear — 1.29, 1.40, 1.80, 0.224; Ridge — 1.31, 1.43, 1.82, 0.207; robust Huber — 1.24, 1.33, 1.76, 0.260; Gradient Boosting — 1.25, 1.37, 1.78, 0.243. Highlight Huber as research champion but keep the age-band baseline operational because Huber fails rolling-origin stability and the 95% classification gate.
...
> Add one slide titled “Low R² changes the claim—it does not erase all value.” Use a large central statement: “Operational recovery R² = 0.222; weight R² = 0.126. Most outcome variation remains unexplained.” Below it, show three columns: What is still useful — MAE is in business units, forecasts update with current evidence, rules and actions remain traceable; Why R² is low — only 5–6 historical cycles, 25–31 independent outcomes, proxy recovery labels, inconsistent weight sampling, missing health/feed/intervention data; Honest positioning — validated experimental prototype, ranges not guarantees, transparent fallbacks stay operational, collect more standardized cycles before production deployment.
...
> Audit the entire revised deck for internal consistency. The only approved targets are 95% recovery and 1,800 g on Day 35. Day 14 is the early-warning checkpoint, not the end of monitoring. The current recovery method is the age-band remaining-loss fallback; Huber is research-only. The current weight method is historical remaining gain. Delete claims of 15 g error, R² around 0.60, guaranteed revenue, causal interventions, or the old mortality-trend/peer-comparison risk system. Keep the final deck concise and readable from a projector.

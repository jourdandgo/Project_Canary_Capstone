    ...
    "manifest": {
        ...
        "blocks": [
            ...
            {"id": "association-detail", "type": "table", "tableId": "association-table"},
            {
                "id": "practicality",
                "type": "markdown",
                "body": "## Canary is practical for triage and directional planning—with clear limits\n\n**Rules-based risk:** deterministic, inspectable and actionable. It answers which building deserves attention and why; it is not a probability.\n\n**Recovery forecast:** predicts additional loss after the review date, then subtracts it from current survival. Constrained Extra Trees is the selected continuous-estimate model because it improves strict whole-cycle error and rolling-origin performance. It still fails the balanced 95% hit/miss gate, so the output is an experimental estimate and range—not a target-hit probability.\n\n**Day 35 weight forecast:** predicts remaining gain. No learned candidate beat the transparent historical remaining-gain baseline on unseen cycles, so the baseline remains operational. More verified cycles and standardized bodyweight measurements are the main path to stronger reliability.",
            },

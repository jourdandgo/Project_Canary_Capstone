...
def _target_metrics(
    ...
    base = {
        ...
        "below_target_recall": float(np.mean(~predicted_hit[~hit])) if (~hit).any() else np.nan,
        "at_or_above_target_recall": float(np.mean(predicted_hit[hit])) if hit.any() else np.nan,
        "confusion_matrix": {
...
def train_recovery_remaining_loss(rows: pd.DataFrame) -> StrengthenedResult:
    ...
    research = next(name for name in simplicity if name in (gate_eligible or eligible))
    ...
    manifest = {
        ...
        "limitations": [
            "Only 25 independent building-cycle recovery outcomes are available.",
...
def train_weight_remaining_gain(rows: pd.DataFrame, base_features: pd.DataFrame) -> StrengthenedResult:
    ...
    manifest = {
        ...
        "model_version": "day35-weight-2.0.0",
        "selected_model": selected,
        "research_champion": research,
        "operational_model": selected,
        "model_kind": "fitted" if model is not None else "formula",
        ...
        "limitations": [
            "Only 31 independent Day 35 building outcomes are available.",
